...
depths = np.linspace(2,20,num=10,dtype=np.int64)
FT_QPs = [[23,24,25],[28,29,30],[33,34,35],[38,39,40]]

#one parameter per QP
best_parameters = []
out_qps = []
for train,valid,ft_qps in zip(trains,valids,FT_QPs):
    for ft_qp in ft_qps:
        data = Data()
        data.load_data(train,valid,ft_qp=ft_qp,balanced=False)
        ...
        out_qps.append(ft_qp)

for best_parameter,out_qp in zip(best_parameters,out_qps): print(out_qp, best_parameter)
